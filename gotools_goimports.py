...
# class GotoolsGoimportsOnSave(sublime_plugin.EventListener):

#   def on_pre_save(self, view):
#     if not GoBuffers.is_go_source(view): return
#     if not setting_value("format_on_save")[0]: return
#     view.run_command('gotools_goimports')

class GotoolsGoimports(sublime_plugin.TextCommand):
  ...
  def run(self, edit):
    ...
    if setting_value("format_backend")[0] == "both":
      command = "gofmt"
      args = ["-e", "-s"]
